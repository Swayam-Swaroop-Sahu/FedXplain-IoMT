...
RESULTS_DIR = "results"
MODEL_PATH = "models/fedprox_global.pth"
...
def main() -> None:
    ...
    # 1. Load FedProx global model
    input_dim = get_input_dim("wifi")
    model = IoMTMLP(input_dim)
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(f"Model file not found at {MODEL_PATH}. Please run src/run_federated_prox.py first.")
    ...
    print("Computing SHAP explanations per client for FedProx model...\n")
    ...
    # ------------------------------------------------------------------
    # 2. Per-client plots: horizontal bar chart of top 10 features (FedProx)
    # ------------------------------------------------------------------
    for proto in PROTOCOLS:
        ...
        bars = ax.barh(y_pos, vals_rev, color="#0284c7", edgecolor="#0369a1", alpha=0.85)
        ax.set_yticks(y_pos)
        ax.set_yticklabels(feats_rev, fontsize=9)
        ax.set_xlabel("Mean |SHAP Value|", fontsize=10, fontweight="bold")
        ax.set_title(f"Top 10 SHAP Features - {proto.upper()} Client (FedProx, Attack Samples)", fontsize=11, fontweight="bold")
        ...
        plt.tight_layout()
        plot_path = os.path.join(RESULTS_DIR, f"shap_top10_{proto}_fedprox.png")
        plt.savefig(plot_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"Saved: {plot_path}")

    # ------------------------------------------------------------------
    # 3. Cross-client comparison plot: top 5 features side by side (FedProx)
    # ------------------------------------------------------------------
    ...
    if valid_protos:
        ...
        fig.suptitle("Cross-Client SHAP Explanation Divergence (FedProx Global Model)", fontsize=13, fontweight="bold", y=1.02)
        plt.tight_layout()
        comparison_plot_path = os.path.join(RESULTS_DIR, "shap_cross_client_comparison_fedprox.png")
        plt.savefig(comparison_plot_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"Saved: {comparison_plot_path}")

    # ------------------------------------------------------------------
    # 4. Top-5 Jaccard overlap (FedProx)
    # ------------------------------------------------------------------
    ...
    report_md_path = os.path.join(RESULTS_DIR, "divergence_report_fedprox.md")
    with open(report_md_path, "w", encoding="utf-8") as f:
        f.write("# SHAP Cross-Client Explanation Divergence Report (FedProx)\n\n")
    ...
    print("FedProx SHAP explanation pipeline completed successfully.")
