"""Federated FedAvg orchestration with per-protocol IoMT clients."""
...
def train_fedavg(
    ...
    """Run FedAvg training loop across all clients."""
    set_seed(seed)
    ...
    print(f"Starting FedAvg: {n_rounds} rounds, {n_epochs} local epochs, "
    ...
    for rnd in range(1, n_rounds + 1):
        ...
        for cid, proto in enumerate(PROTOCOLS):
            ...
            updated_weights, n_train, metrics = client.fit(
                global_weights, {"epochs": n_epochs}
            )
    ...
    if save_path is None:
        ...
        save_path = os.path.join(MODELS_DIR, f"fedavg_global_seed{seed}.pth" if seed != 42 else "fedavg_global.pth")
